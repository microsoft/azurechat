...
type FeatureFormProps = {
  formValues: FeatureModel
  onSubmit: (feat: FeatureModel) => Promise<void>
  onDelete?: (featureId: string) => Promise<void>
  onClose?: () => void
}
export default function FeatureForm({ formValues, onSubmit, onDelete, onClose }: FeatureFormProps): JSX.Element {
...
const DeleteFeatureDialog: React.FC<{
  name: string
  loading: boolean
  ...
  const handleClick = async (): Promise<void> => {
    try {
      ...
      logger.error(`Error deleting smart feature: ${name}`, { error })
    }
  }
  return (
    <div
      ...
      <div className="mx-auto w-full max-w-lg overflow-hidden rounded-lg bg-background p-4">
        ...
        <div className="flex justify-end">
          ...
          <Button
            ...
            onClick={handleClick}
            disabled={loading}
            ariaLabel="Confirm delete feature"
          >
            Delete feature
          </Button>
        </div>
