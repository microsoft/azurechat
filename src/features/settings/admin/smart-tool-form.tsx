...
type SmartToolFormProps = {
  formValues: SmartToolModel
  onSubmit: (value: SmartToolModel) => Promise<void>
  onDelete?: (id: string) => Promise<void>
  onClose?: () => void
}
export default function SmartToolForm({ formValues, onSubmit, onDelete, onClose }: SmartToolFormProps): JSX.Element {
  ...
  return (
    <>
      ...
      <Form.Root className="flex flex-col gap-2" onSubmit={handleSubmit}>
        <Form.Field name={"id"} className="grid grid-cols-[minmax(auto,150px),1fr] items-center gap-4">
          ...
          <Form.Control asChild>
            <Input
              ...
              name="id"
              className="w-full"
              placeholder="Smart tool id"
              aria-label="Smart tool id"
        ...
        <Form.Field name={"name"} className="grid grid-cols-[minmax(auto,150px),1fr] items-center gap-4">
          ...
          <Form.Control asChild>
            <Input
              ...
              name="name"
              className="w-full"
              placeholder="Smart tool name"
              aria-label="Smart tool name"
        ...
        <Form.Field name={"description"} className="grid grid-cols-[minmax(auto,150px),1fr] items-center gap-4">
          ...
          <Form.Control asChild>
            <Input
              ...
              name="description"
              className="w-full"
              placeholder="Smart tool description"
              aria-label="Smart tool description"
              disabled={isSubmitting}
        ...
        <Form.Field name={"template"} className="grid grid-cols-[minmax(auto,150px),1fr] items-start gap-4">
          <Form.Label htmlFor={"template"} className="pt-1 text-right">
            Template:
          </Form.Label>
          <Form.Control asChild>
            <Textarea
              ...
              placeholder="Smart tool template"
              aria-label="Smart tool template"
              rows={4}
              disabled={isSubmitting}
        ...
        <div className="mb-4 flex justify-end gap-4">
          ...
          <Form.Submit asChild>
            ...
            >
              {isSubmitting ? "Processing..." : `${isCreating ? "Create Tool" : "Save changes"}`}
            </Button>
          </Form.Submit>
        </div>
      </Form.Root>
...
const DeleteSmartToolDialog: React.FC<{
  name: string
  loading: boolean
  ...
  const handleClick = async (): Promise<void> => {
    try {
      ...
      logger.error(`Error deleting smart tool: ${name}`, { error })
    }
  }
  return (
    <div
      ...
      <div className="mx-auto w-full max-w-lg overflow-hidden rounded-lg bg-background p-4">
        <div className="mb-4">
          <Typography variant="h4" className="text-foreground" id="dialog-title">
            Are you sure you want to delete &quot;{name}&quot; smart tool?
          </Typography>
        </div>
        <div className="mb-4">
        ...
        <div className="flex justify-end">
          ...
          <Button
            ...
            onClick={handleClick}
            disabled={loading}
            ariaLabel="Confirm delete smart tool"
          >
            Delete Smart Tool
          </Button>
        </div>
